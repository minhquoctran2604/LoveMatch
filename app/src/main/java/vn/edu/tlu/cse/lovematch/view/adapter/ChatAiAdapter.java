...
public class ChatAiAdapter extends RecyclerView.Adapter<ChatAiAdapter.MessageViewHolder> {

    private static final int VIEW_TYPE_USER = 1;
    private static final int VIEW_TYPE_AI = 2;

    private final List<MessageAI> messageList;
    private long lastMessageTimestamp = -1;

    public ChatAiAdapter(List<MessageAI> messageList) {
    ...
    public void onBindViewHolder(@NonNull MessageViewHolder holder, int position) {
        ...
        if (message.isUserMessage()) {
            Glide.with(holder.itemView.getContext())
                    .load(R.drawable.gai1) // Ảnh đại diện của người dùng
